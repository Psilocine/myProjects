(function(){
  ...
  function renderHtml() {
    ...
    var oTitleBox = document.createElement("div");  // 日历标题 设置上一月 下一月图标
    var oBodyBox = document.createElement("div");  // 表格区 显示数据


    // 设置标题盒子中的html
    oTitleBox.className = 'calendar-title-box';
    oTitleBox.innerHTML = "<h3 class='year' id='year'>2017<i></i></h3> <div class='month-week-box'><span class='prev' id='prev'>&lt;</span><h3 class='month' id='month'><span>08月</span></h3>" + 
                          "<span id='next' class='next'>&gt;</span> <ul class='week'><li>日</li><li>一</li><li>二</li><li>三</li><li>四</li>" +
  ...
  function showCalendarData() {
    var nYear = dateObj.getDate().getFullYear();
    var nMonth = dateObj.getDate().getMonth() + 1;
    var oDateStr = getDateStr(dateObj.getDate());
    // 日历标题栏中的年、月
    ...
    for(var i = 0; i < aTd.length; i++) {
      ...
      if(nThisDayStr == getDateStr(new Date())) {    // 当前天
        aTd[i].children[0].className = 'currentDay';
        ...
        if(nThisDay.getDay() == 0 || nThisDay.getDay() == 6) {
          ...
          aTd[i].children[0].className = 'currentMonth';
        }
      }else {   
        aTd[i].children[0].className = 'otherMonth';
      }
    }
  }
  /**
